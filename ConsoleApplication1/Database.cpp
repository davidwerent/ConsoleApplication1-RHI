...
Database::Database(string IP, string login, string password, string dbname, int port)
{
	...
	if (conn) {
		cout << "Connection with server is good!\n"; isConnect = true;
	}
	else {
...
void Database::LoadClientFromTable(string _login, string _password, int _role) //вызвать её с введеным пользователем логином и паролем.
{
	...
	if (!qstate)
	{
		...
		{
			//cout << row[0] << "\t" << row[1] << endl;
			string temp(row[1]); // в row[1] находятся типы полей
			if (temp == "int")
			{
				cout << "ITS INT!!!!!!\n";
			...
			//cout << row[0] << endl; в row[0] находятся названия полей
		} cout << endl;
